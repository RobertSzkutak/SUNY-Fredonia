...
class MainHandler(webapp2.RequestHandler):
    ...
    def writeBlog(self):
        ...
        self.response.out.write(blog + posts + "<br> Queried " + str(int(age)) + " seconds ago" + "</body></html>")
...
class PostHandler(webapp2.RequestHandler):
    def get(self, postid):
        if self.request.url.endswith('.json'):
            ...
            self.format = 'html'
        p, age = age_get('BLOGS_' + str(postid))
        if p is None:
            p = Blog.get_by_id(int(postid))
            age_set('BLOGS_' + str(postid), p)
            p, age = age_get('BLOGS_' + str(postid))
        if not p:
            self.error(404)
            return
        if self.format == 'html':
            self.response.out.write(post % { "subject" : str(p.subject), "content": str(p.content), "date": str(p.date), "age": 'Queried ' + str(int(age)) + ' seconds ago'} )
        if self.format == 'json':
...
app = webapp2.WSGIApplication([('/?(?:.json)?', MainHandler), ('/welcome', WelcomeHandler), ('/signup', SignupHandler), ('/login', LoginHandler), ('/logout', LogoutHandler), ('/newpost', NewPostHandler), ('/flush', FlushHandler), ('/([0-9]+)(?:.json)?', PostHandler)], debug=True)
